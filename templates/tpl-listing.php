...
<!-- Business Header -->
<div id="business-header" class="container-fluid bg-default p-3 mb-5 mt-5">
	<!-- Breadcrumbs -->
			...
			<div class="col-12 col-md-7">
				<div class="row">
					...
					<div class="col-8 col-lg-9">
						<h1 id="place-id-<?= $place_id ?>"><div class="animated pulse"><?= $place_name ?> <?= $feat ? "<span class='featured-badge'>$txt_featured</span>" : '' ?></h1>
			...
			<div class="col-12 col-md-5 py-2">
				...
				if(!empty($phone)) {
					...
					<div class="business-phone text-md-right text-dark text-nowrap" style="font-size:2rem">
						<a href="tel:<?= $area_code ?><?= $phone ?>"><i class="fas fa-phone fa-spin"></i>
							<?php
				...
				<div class="text-md-right social">
					...
					if(!empty($website)) {
						?>
						<a href="<?= $website_url ?>" target="_blank" class="mr-2"><i class="fas fa-globe fa-spin"></i><b>Open <?= $place_name ?> Site</b><i class="fas fa-mouse-pointer fa-spin"></i> </a>
					<?php
					}
					...
					if(!empty($facebook)) {
						?>
						<a href="https://facebook.com/<?= $facebook ?>" target="_blank" class="mr-2" target="_blank"><i class="fab fa-facebook-f fa-spin" aria-hidden="true"></i></a>
					<?php
					}
					...
					if(!empty($twitter)) {
						?>
						<a href="https://linkedin.com/<?= $twitter ?>" target="_blank" class="mr-2" target="_blank"><i class="fab fa-twitter fa-spin" aria-hidden="true"></i></a>
					<?php
					}
					?>

					<a href="#" id="shareDropdown" class="mr-2" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-share-alt fa-spin"></i>
					</a>

					<div class="dropdown-menu dropdown-menu-right" aria-labelledby="shareDropdown">
						<a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=<?= urlencode($canonical) ?>&src=sdkpreparse" class="dropdown-item"><i class="fab fa-facebook-f fa-spin"></i> Facebook</a>

						<div class="dropdown-divider"></div>

						<a class="dropdown-item" href="https://twitter.com/intent/tweet?url=<?= urlencode($canonical) ?>&text=<?= urlencode($place_name) ?>"><i class="fab fa-twitter fa-spin"></i> Twitter</a>
	...
	<div class="row">
		<div class="col-md-6 col-lg-8">
					...
					<div class="owl-carousel owl-theme">
						...
						foreach($photos as $k => $v) {
							...
							<div class="" style="width:100%;height:320px"
								data-dot=
									"
									<img src='<?= $v['img_url'] ?>' alt='image' width='120' class='rounded m-2'>
									">
				...
				<div class="mb-5">
					...
					foreach($custom_fields as $k => $v) {
						if(!(($v['field_type'] == 'radio' || $v['field_type'] == 'select') && $v['values_list'] == $cfg_custom_field_toggle_values)) {
							...
							if($i == 0) {
								?>
								<p class="text-dark text-uppercase" style="font-weight:600"><?= $txt_more_details ?></p>
								<hr>
								<?php
							}
							?>
							<div class="container-fluid mb-2">
								<div class="row <?= $i % 2 == 0 ? 'bg-default' : '' ?> p-3">
									<div class="col-sm-6">
										<strong><?= $v['icon'] ?> <?= !empty($v['tr_field_name']) ? $v['tr_field_name'] : $v['field_name'] ?>:</strong>
									</div>
			...
			<div id="reviews" class="tab-pane" role="tabpanel">
				<p class="text-dark text-uppercase" style="font-weight:600"><?= $txt_reviews ?><small> <text style="color:#f6b21a;"> | Give honest review & win shopping voucher</small></text>
				<hr>
