...
$(".comments_span").click(function () {
      
    var ads_id     = $(this).attr('data-ads-id');
    // var comment_id = $(this).attr('data-comment-id');
    var page_num   = 0;
    // get_comments(ads_id,comment_id,page_num);
    get_comments(ads_id);
   
});


function get_comments(ads_id){
	$.ajax({
			type: 'POST',
			url: '<?= $baseurl ?>/ajax-ads-comment.php',
		    dataType:'json',
			data : {ads_id:ads_id,
			        ajax_type:"get_comment"
			        
			},
			
			success: function(json){
	
			    var comment_div__ID ="#comment_div_"+ads_id;
			    var ads_tr_ID ="#ads_tr_"+ads_id;
			     $(comment_div__ID).html("");
			    for(var i = 0; i<json.length ;i++){
			       
			        var html = '<div class="col-md-12 root-comments" id="">'+
                                '<div class="comment"> '+
                                    '<div class="show-comment">'+
                                        '<div class="body showmore">'+
                                            '<div class="showmore_content" style="height: 50px;">'+
                                                '<div class="actions">'+
                                                    '<div>16 hours ago</div>'+
                                                '</div>'+
                                                '<span style="color:#004998">'+json[i]['first_name']+ " "+json[i]['last_name']+'</span>'+
                                                '<p>'+json[i]['cmt_message']+
                                                '</p>'+
                                            '</div>'+
                                ...
                                '</div>'+
                            '</div>';
                 $(comment_div__ID).append(html);
                
			    }
			    
			    if($(ads_tr_ID).hasClass("in")){
                  
			         $(ads_tr_ID).removeClass("in");
			    }else{

			        $(ads_tr_ID).addClass("in");
			    }
...
function add_comments(ads_id){
     var comment_body_ID ="#comment_body_"+ads_id;
     var comment_message = $(comment_body_ID).val();
