...
$ads_list = array();
$test =array();
$query = " SELECT * FROM ads left JOIN cats on ads_category = cats.id left JOIN users on users.id = ads_maker_id  where cats.parent_id = :cat_id_1 or cats.id = :cat_id_2";
$stmt = $conn->prepare($query);
$stmt->bindValue(":cat_id_1", $cat_id);
$stmt->bindValue(":cat_id_2", $cat_id);
$stmt->execute();

while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    
    
    $query = " SELECT count(*)as ads_comment_cnt FROM ads_comment where ads_comment.cmt_ads_id = :ads_id";
    $stmt1 = $conn->prepare($query);
	$stmt1->bindValue(":ads_id", $row['ads_id']);
	$stmt1->execute();
	$comment_row = $stmt1->fetch(PDO::FETCH_ASSOC);
	$ads_comment_cnt = $comment_row['ads_comment_cnt'];
	
	$ads_id              = $row['ads_id'];
	$ads_subject         = !empty($row['ads_subject'    ]) ? $row['ads_subject'    ] : '';
	$ads_message         = !empty($row['ads_message'    ]) ? $row['ads_message'    ] : '';
	$ads_type            = !empty($row['ads_type'       ]) ? $row['ads_type'       ] : '0';
	$ads_date            = !empty($row['ads_date'       ]) ? $row['ads_date'       ] : '';
	$ads_visitor_cnt     = !empty($row['ads_visitor_cnt']) ? $row['ads_visitor_cnt'] : '0';
    $first_name          = !empty($row['first_name'     ]) ? $row['first_name'     ] : '';
    $last_name           = !empty($row['last_name'      ]) ? $row['last_name'      ] : '';
	$ads_company         = !empty($row['ads_company'    ]) ? $row['ads_company'    ] : '';
    $ads_comment_cnt     = !empty($ads_comment_cnt       ) ? $ads_comment_cnt : '0';

	$ads_list[] = array(
		'ads_id'          => $ads_id,
		'ads_subject'     => $ads_subject,
		'ads_message'     => $ads_message,
		'ads_type'        => $ads_type,
		'ads_date'        => $ads_date,
		'ads_visitor_cnt' => $ads_visitor_cnt,
		'first_name'      => $first_name,
		'last_name'       => $last_name,
		'ads_company'     => $ads_company,
		'ads_comment_cnt' => $ads_comment_cnt
	);
