...
if($post_token == $session_token || $post_token == $cookie_token) {
	try {
		...
		if($cfg_enable_sitemaps) {
			$this_listing_link = get_listing_link($place_id, '', $category_id, '', $city_id, '', '', $cfg_permalink_struct);
			sitemap_update_lastmod($this_listing_link);
		}
	}
...
if(!empty($mail_after_post)) {
	...
	$listing_slug = !empty($row['slug']) ? $row['slug'] : '';

	$edited_listing_url = get_listing_link($place_id, $listing_slug, $category_id, $cat_slug, $city_id, $city_slug, $state_slug, $cfg_permalink_struct);
