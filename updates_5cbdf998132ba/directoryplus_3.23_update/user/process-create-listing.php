...
if($post_token == $session_token || $post_token == $cookie_token) {
	...
	if($cfg_enable_sitemaps && $status == 'approved' && $paid == 1) {
		...
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this_city_slug = $row['slug'];

		$query = "SELECT slug FROM states WHERE state_id = :state_id";
		$stmt = $conn->prepare($query);
		$stmt->bindValue(':state_id', $state_id);
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this_state_slug = $row['slug'];
		...
		$this_link_url = $baseurl . '/'. $route_listing . '/' . $this_state_slug . '/' . $this_city_slug . '/' . $this_cat_slug  . '/' . $place_slug;
		sitemap_add_url($this_link_url);
	}
}

// else probably user reloaded page
else {
