		...
		<div class="col-md-8 col-lg-9">
			...
			<div class="edit-custom-field">
				<form class="form-edit-custom-field" method="post">
					...
					if(!empty($cfg_languages) && is_array($cfg_languages)) {
						foreach($cfg_languages as $v) {
							if(!empty($custom_field_lang[$v])) {
								$field_name = isset($custom_field_lang[$v]['field_name']) ? $custom_field_lang[$v]['field_name'] : $field_name;
							}
							?>
							<div class="form-group">
								<label class="label" for="custom_field_lang_<?= $v ?>">
									<strong><span class="badge badge-warning"><?= $iso_639_1_native_names[$v] ?></span>: <?= $txt_field_name ?></strong>
								</label>
					...
					<div class="form-group">
						<label class="label" for="field_type"><?= $txt_filter_display ?></label>
					...
					<div class="form-group">
						<label class="label" for="tooltip"><?= $txt_tooltip ?></label>
						...
						foreach($cfg_languages as $v) {
							...
							<div class="form-group">
								<label class="label" for="tooltip_lang_<?= $v ?>">
									<strong><span class="badge badge-warning"><?= $iso_639_1_native_names[$v] ?></span>: <?= $txt_tooltip ?></strong>
								</label>
					...
					<div class="form-group">
						<label class="label" for="icon"><?= $txt_icon ?></label>
						<input type="text" id="icon" class="form-control" name="icon" value="<?= $icon ?>">
					</div>

					<div class="form-group">
						<label class="label" for="icon"><?= $txt_options ?></label>
					...
					if(!empty($cfg_languages) && is_array($cfg_languages)) {
						foreach($cfg_languages as $v) {
							?>
							<div class="form-group">
								<label class="label" for="values_list"><span class="badge badge-warning"><?= $iso_639_1_native_names[$v] ?>:</span> <strong><?= $txt_values_list ?></strong></label>
								<input type="text" id="values_list" class="form-control" name="values_list_lang[<?= $v ?>]" value="<?= isset($custom_field_lang[$v]['values_list']) ? $custom_field_lang[$v]['values_list'] : '' ?>">
							</div>
							<?php
						}
					}
					...
					<div class="form-group">
						<label class="label" for="field_order"><?= $txt_field_order ?></label>
					...
					<div class="form-group">
						...
						<input type="checkbox" id="select_all" name="select_all"> <?= $txt_select_all ?><br>
