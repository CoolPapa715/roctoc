...
$query = "SELECT
			r.field_value,
			f.field_id, f.field_name, f.field_type, f.values_list, f.tooltip, f.icon, f.required, f.field_order,
...
// get list of custom fields ids
$custom_fields_ids = array_column($custom_fields, 'field_id');
...
if(!empty($custom_fields_ids)) {
	...
	$query = "SELECT tr.*, cf.field_name AS cf_field_name
				FROM translation_cf tr
				LEFT JOIN custom_fields cf ON cf.field_id = tr.field_id
				WHERE tr.field_id IN($custom_fields_ids)";
	$stmt = $conn->prepare($query);
	$stmt->execute();

	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		$this_tr_field_id   = !empty($row['field_id'     ]) ? $row['field_id'     ] : '';
		$this_tr_field_name = !empty($row['field_name'   ]) ? $row['field_name'   ] : '';
		$this_cf_field_name = !empty($row['cf_field_name']) ? $row['cf_field_name'] : '';
		$this_values_list   = !empty($row['values_list'  ]) ? $row['values_list'  ] : '';
		$this_lang          = !empty($row['lang'         ]) ? $row['lang'         ] : '';

		// put values in array
		$this_values_list = explode(';', $this_values_list);

		// add to translation array
		$custom_fields_translated[$this_cf_field_name] = array(
