...
//
if(!empty($place_id)) {
	// the first subquery (s1) gets all global fields
	// the second subquery (s2) gets the values for this place for each custom field from s1
	$query = "SELECT s1.*, s2.field_value
			FROM
				(
				SELECT cf.*, tr.field_name AS tr_field_name, tr.values_list AS tr_values_list
...
if(!empty($place_id) && !empty($cat_id)) {
	...
	$query = "SELECT s1.*, s2.field_value FROM
				...
				SELECT cf.*, tr.field_name AS tr_field_name, tr.values_list AS tr_values_list
					...
					LEFT JOIN translation_cf tr ON cf.field_id = tr.field_id AND tr.lang = :html_lang
					WHERE rc.cat_id = :cat_id AND cf.field_status = 1
				) s1
			LEFT JOIN
				(
				SELECT place_id, field_id AS field_id2, GROUP_CONCAT(field_value SEPARATOR ':::') AS field_value
	...
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		$field_value = !empty($row['field_value']) ? $row['field_value'] : 0;

		// field translation values
		$this_tr_field_name = !empty($row['tr_field_name']) ? $row['tr_field_name'] : $field_name;
		$this_tr_values_list = !empty($row['tr_values_list']) ? $row['tr_values_list'] : $values_list;
