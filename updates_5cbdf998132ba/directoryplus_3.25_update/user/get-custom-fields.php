	...
	foreach($custom_fields as $v) {
		...
		if($field_type == 'radio' || $field_type == 'select' || $field_type == 'checkbox') {
			...
			foreach($values_arr as $k => $v) {
				if(empty($tr_values_arr[$k])) {
					$tr_values_arr[$k] = $values_arr[$k];
				}
			}
		}
			...
			<div class="mb-3">
				...
				foreach($values_arr as $k => $v) {
					...
					<div class="form-check form-check-inline">
						...
						<label for="field_<?= $field_id ?>_<?= $i ?>" class="form-check-label"><?= $tr_values_arr[$k] ?></label>
					</div>
					<?php
					$i++;
				}
		...
		if($field_type == 'select') {
			...
			<div class="form-group">
				<label for="field_<?= $field_id ?>"><?= $tr_field_name ?> <?= $tooltip ?></label>
				<select id="field_<?= $field_id ?>" class="form-control" name="field_<?= $field_id ?>" <?= $required ?>>
				<?php
				foreach($values_arr as $k => $v) {
					$v = e(trim($v));
					?>
					<option value="<?= $v ?>"><?= isset($tr_values_arr[$k]) ? $tr_values_arr[$k] : $v ?></option>
					<?php
				}
				?>
				</select>
