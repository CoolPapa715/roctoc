...
if($total_rows > 0) {
	...
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		$this_cat_slug          = !empty($row['cat_slug'      ]) ? $row['cat_slug'      ] : 'cat';
		...
		$this_place_name        = !empty($row['place_name'    ]) ? $row['place_name'    ] : 'Undefined';
		...
		$this_review_city_id    = !empty($row['review_city_id']) ? $row['review_city_id'] : null;
		$this_review_city_name  = !empty($row['city_name'     ]) ? $row['city_name'     ] : null;
		$this_review_city_slug  = !empty($row['city_slug'     ]) ? $row['city_slug'     ] : null;
		$this_review_state_slug = !empty($row['state_slug'    ]) ? $row['state_slug'    ] : 'state';
		...
		$this_review_city_id    = e($this_review_city_id   );
		$this_review_city_name  = e($this_review_city_name );
		$this_review_city_slug  = e($this_review_city_slug );
		$this_review_state_slug = e($this_review_state_slug);
		$this_text              = e($this_text             );

		// link to the place's page
		$this_link_url = $baseurl . '/listing/' . $this_review_state_slug . '/' . $this_review_city_slug . '/' . $this_cat_slug . '/' . $this_place_slug;
