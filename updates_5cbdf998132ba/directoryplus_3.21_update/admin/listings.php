...
$page_url = "$baseurl/admin/listings?s=$s&cat=$cat_id&sort=$sort&page=";
...
// if no search term
if(empty($s)) {
	$query = "SELECT COUNT(*) AS total_rows FROM places LEFT JOIN rel_place_cat ON places.place_id = rel_place_cat.place_id WHERE TRUE";
}

// else with search term
else {
	$query = "SELECT COUNT(*) AS total_rows FROM places LEFT JOIN rel_place_cat ON places.place_id = rel_place_cat.place_id WHERE MATCH(place_name, description, short_desc) AGAINST (:s)";
}

// append cat id clause
if(!empty($cat_id)) {
...
if($total_rows > 0) {
	...
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		// link to each place
		$this_link_url = $baseurl . '/listing/' . $this_state_slug . '/' . $this_city_slug . '/' . $this_cat_slug  . '/' . $this_place_slug;
