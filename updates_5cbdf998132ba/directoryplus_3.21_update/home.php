...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
	...
	// img path
	$cat_img_path = $pic_basepath . '/category/cat-' . $row['id'];
	...
	if(!empty($user_cookie_lang)) {
		$cur_loop['cat_name'] = cat_name_transl($row['id'], $user_cookie_lang, 'singular', $row['name']);
	}
...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
	...
	// link
	$feat_place_link = $baseurl . '/listing/' . $feat_state_slug . '/' . $feat_city_slug . '/' . $feat_cat_slug . '/' . $feat_place_slug;
...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
	...
	// link
	$latest_place_link = $baseurl . '/listing/' . $latest_state_slug . '/' . $latest_city_slug . '/' . $latest_cat_slug . '/' . $latest_place_slug;
...
if(!empty($geo_city_id)) {
	...
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		// link
		$near_place_link = $baseurl . '/listing/' . $near_state_slug . '/' . $near_city_slug . '/' . $near_cat_slug . '/' . $near_place_slug;
