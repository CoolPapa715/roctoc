...
if($post_token == $session_token || $post_token == $cookie_token) {
	try {
		...
		if($cfg_enable_sitemaps) {
			$this_link_url = get_listing_link($place_id);
			sitemap_update_lastmod($this_link_url);
		}
	}
...
if(!empty($mail_after_post)) {
	...
	$edited_listing_url = $baseurl . '/listing/' . $state_slug . '/' . $city_slug . '/' . $cat_slug . '/' . $listing_slug;
