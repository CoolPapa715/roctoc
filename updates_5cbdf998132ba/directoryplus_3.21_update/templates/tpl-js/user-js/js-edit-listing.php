		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.result == 'success') {
					...
					var logo_img = '<img src="' + data.message + '" width="132">';
...
(function() {
	...
	$('#item_img').on('change', function(e) {
		...
		$.each(files, function(i, file) {
			...
			var random_div_id = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
			...
			$.ajax({
				...
				beforeSend : function() {
					// Add preloader
					$('<div id="' + random_div_id + '" class="thumbs-preloader mr-3"><i class="fas fa-spinner fa-spin"></i></div>').appendTo('#uploaded');
				},
				success: function(response){
					...
					// upload success
						...
						if($('.thumbs').length < <?= $max_pics ?>) {
							...
							$('<div class="btn-light delete_pic"><small><?= $txt_delete ?></small></div>').appendTo(temp_thumb_div);
