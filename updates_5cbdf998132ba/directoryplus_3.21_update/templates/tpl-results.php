		...
		<form id="sidebar-filter" class="p-2" method="get" action="<?= $baseurl ?>/results">
			...
			foreach($custom_fields_sidebar as $k => $v) {
				...
				<div class="mb-3" id="li-field-<?= $v['field_id'] ?>">
					<p><strong><?= $v['field_name'] ?></strong></p>
					...
					if($v['filter_display'] == 'radio') {
						foreach($values_arr as $k2 => $v2) {
							...
							<div class="custom-control custom-radio">
								...
								<label class="custom-control-label" for="val_<?= $k ?><?= $k2 ?>"><?= $v2 ?></label>
							</div>
							<?php
						}
					}
					...
					elseif($v['filter_display'] == 'range_text') {
						...
						?>
						<div class="form-group">
							<input type="text" id="val_<?= $k ?><?= $k2 ?>" class="form-control" name="field_<?= $v['field_id'] ?>[from]" value="<?= $val_from ?>">
					...
					elseif($v['filter_display'] == 'range_number') {
						...
						<div class="form-group">
							...
							<input type="number" id="val_<?= $k ?><?= $k2 ?>" class="form-control mb-1" name="field_<?= $v['field_id'] ?>[to]" placeholder="<?= $txt_to ?>" value="<?= $val_to ?>">
						</div>
						<?php
					}
					...
					elseif($v['filter_display'] == 'range_select') {
						...
						?>
						<div class="form-group">
							<select class="form-control" name="field_<?= $v['field_id'] ?>[from]">
								<option value=""></option>
								<?php
								foreach($values_arr as $k2 => $v2) {
									$v2 = e(trim($v2));
					...
					// else assume it's text
						...
						?>
						<div class="form-group">
							<input type="text" class="form-control" name="field_<?= $v['field_id'] ?>" value="<?= $custom_fields[$v['field_id']]['field_value'] ?>">
						</div>
						<?php
					}
					?>
		...
		<div class="col-lg-7">
			...
			<div class="container-fluid item-list">
				...
				foreach($list_items as $k => $v) {
					$place_link = "$baseurl/listing/" . $v['state_slug'] . "/" . $v['city_slug'] . "/" . $v['cat_slug'] . "/" . $v['listing_slug'];
						...
						<div class="flex-grow-1 p-3 pl-4">
							<div class="d-flex mb-3">
								<div class="flex-grow-1">
									<h4 class="mb-2"><a href="<?= $place_link ?>"><?= $v['place_name'] ?></a>
										<?= $feat_badge ?>
									</h4>
							...
							<div class="d-flex">
								<div class="address flex-grow-1">
									...
									if(!empty($v['area_code']) && !empty($v['phone'])) {
										?>
										<div class="tel">
											<a href="tel:<?= $v['area_code'] ?>-<?= $v['phone'] ?>"><strong><i class="fas fa-mobile-alt"></i> <?= $v['area_code'] ?>-<?= $v['phone'] ?></strong></a>
										</div>
										<?php
									}
									?>
