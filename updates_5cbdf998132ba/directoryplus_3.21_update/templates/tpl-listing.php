	...
	<div class="container breadcrumbs">
		...
		if(!empty($cats_path)) {
			foreach($cats_path as $v) { // $cats_path is an array of category ids sorted hierarchically
				$stmt = $conn->prepare('SELECT name, id, cat_slug FROM cats WHERE id = :cat_id');
				...
				if(!empty($user_cookie_lang)) {
					$this_cat_name = cat_name_transl($this_cat_id , $user_cookie_lang, 'plural', $this_cat_name);
				}
				?>
				 > <a href="<?= $baseurl ?>/listings/<?= $state_slug ?>/<?= $city_slug ?>/<?= $this_cat_slug ?>"><?= $this_cat_name ?></a>
				<?php
			}
		}
		?>
			...
			<div class="col-12 col-md-7">
				<div class="row">
					...
					<div class="col-8 col-lg-9">
						<h1><?= $place_name ?> <?= $feat ? "<span class='featured-badge'>$txt_featured</span>" : '' ?></h1>
						...
						if($place_userid == 1) {
							?>
							<a href="<?= $baseurl ?>/claim?id=<?= $place_id ?>"><small><em><?= $txt_claim ?></em></small></a>
							<?php
						}
						?>
					</div>
			...
			<div class="col-12 col-md-5 py-2">
				...
				if(!empty($phone)) {
					...
					<div class="business-phone text-md-right text-dark text-nowrap" style="font-size:2rem">
						<a href="tel:<?= $area_code ?><?= $phone ?>"><i class="la la-phone"></i><?= $area_code ?> <?= $phone ?></a>
					</div>
				<?php
				}
				?>
...
<div class="container mt-3">
	<div class="row">
		<div class="col-md-6 col-lg-8">
					...
					<div class="owl-carousel owl-theme">
						...
						foreach($videos as $v) {
							...
							<div class="" style="width:100%;height:320px"
								data-dot=
									"
									<img src='<?= $v['thumb'] ?>' width='120' class='rounded m-2'>
									">
			...
			<!-- Features -->
			<?php
			if(!empty($custom_fields_grouped)) {
				$i = 0;
				foreach($custom_fields_grouped as $k => $v) {
					if($v[0]['field_type'] == 'radio' && $v[0]['values_list'] == $cfg_custom_field_toggle_values) {
						...
						if($v[0]['field_value'] == explode(';', $cfg_custom_field_toggle_values)[0]) {
							?>
		...
		<div class="col-md-6 col-lg-4">
			...
			<div class="mb-4">
				...
				<button type="button" class="btn-favorites btn btn-block btn-outline-dark" data-place-id="<?= $place_id ?>"><i class="<?= $is_fave ? 'fas' : 'far' ?> fa-heart"></i> <?= $txt_add_to_favorites ?></button>
			</div>
