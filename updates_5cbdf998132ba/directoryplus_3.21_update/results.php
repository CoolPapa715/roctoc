...
$query = "SELECT * FROM cats WHERE parent_id = 0 AND cat_status = 1";
$stmt  = $conn->prepare($query);
$stmt->execute();
...
/*--------------------------------------------------
Current category's children (only if top level category)
--------------------------------------------------*/
...
/*
$custom_fields is Custom fields from $_GET vars (note that field_type and field_name indices where added to the $custom_fields arr later by querying the database)
print_r2($custom_fields);
...
SELECT COUNT(*) AS total_rows
	FROM (SELECT COUNT(*) AS total_count, p.place_id
			FROM places p
			...
			WHERE r.cat_id IN (171)
				...
				AND p.paid = 1
				AND
					((rpcf.field_id = 30 AND rpcf.field_value = 'Manual')
					OR (rpcf.field_id = 27 AND rpcf.field_value = 'Toyota'))
			GROUP BY p.place_id
			HAVING total_count = 2) subq1
*/
...
if($total_rows > 0) {
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		$this_listing_city_name  = !empty($row['city_name'   ]) ? $row['city_name'   ] : '';
		$this_listing_city_slug  = !empty($row['slug'        ]) ? $row['slug'        ] : '';
		$this_place_name         = !empty($row['place_name'  ]) ? $row['place_name'  ] : '';
		$this_listing_slug       = !empty($row['listing_slug']) ? $row['listing_slug'] : '';
		$this_listing_state_abbr = !empty($row['state'       ]) ? $row['state'       ] : '';
		$this_listing_state_id   = !empty($row['state_id'    ]) ? $row['state_id'    ] : '';
		...
		$this_listing_city_name  = e($this_listing_city_name );
		$this_listing_city_slug  = e($this_listing_city_slug );
		$this_place_name         = e($this_place_name        );
		$this_listing_slug       = e($this_listing_slug      );
		$this_listing_state_abbr = e($this_listing_state_abbr);
		$this_listing_state_id   = e($this_listing_state_id  );
		...
		// link
		$this_listing_link = $baseurl . '/listing/' . $this_state_slug . '/' . $this_listing_city_slug . '/' . $this_cat_slug . '/' . $this_listing_slug;
		...
		$list_items[] = array(
			...
			'cat_name'     => $this_cat_name,
			'cat_slug'     => $this_cat_slug,
			'city_name'    => $this_listing_city_name,
			'city_slug'    => $this_listing_city_slug,
			...
			'state_abbr'   => $this_listing_state_abbr,
...
if(empty($list_items)) {
	...
	$query = "SELECT
		...
		LEFT JOIN cities c ON c.city_id = p.city_id
		LEFT JOIN states s ON c.state_id = s.state_id
		LEFT JOIN rel_place_cat rpc ON rpc.place_id = p.place_id
