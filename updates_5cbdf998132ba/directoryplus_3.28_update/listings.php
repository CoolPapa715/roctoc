...
if($frags_count == 4) {
	...
	*/
	...
	// else route[2] is city
		...
		// get city info
		$query = "SELECT * FROM cities c LEFT JOIN states s ON c.state_id = s.state_id WHERE c.slug = :slug";
		$stmt  = $conn->prepare($query);
		$stmt->bindValue(':slug', $route[2]);
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
...
if($total_rows > 0) {
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		// link
		$this_listing_link = get_listing_link($this_place_id, $this_place_slug, $this_cat_id, $this_cat_slug, '', $this_city_slug, $this_state_slug, $cfg_permalink_struct);
