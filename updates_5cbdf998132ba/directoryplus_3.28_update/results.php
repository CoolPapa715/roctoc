...
$query = "SELECT * FROM cats WHERE cat_status = 1";
$stmt  = $conn->prepare($query);
$stmt->execute();
...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
	...
	if(!empty($user_cookie_lang)) {
		...
		$this_cat_plural_name = cat_name_transl($this_cat_id , $user_cookie_lang, 'plural', $this_plural_name);
	}
...
if($total_rows > 0) {
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		// link
		$this_listing_link = get_listing_link($this_place_id, $this_listing_slug, $this_cat_id, $this_cat_slug, '', $this_city_slug, $this_state_slug, $cfg_permalink_struct);
